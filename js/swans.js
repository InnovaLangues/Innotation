		...
		case ETATS.LECTURE :
				...
				//toggle d'affichage de l'annotation..rend juste l'annotation invisble
			 	$j("#afficherAnnotation").click(function(){
					if(IS_ANNOTATED){
						if(ETAT == ETATS.ECRITURE) $j("#bloc_accent").hide();
						...
						if(ETAT == ETATS.ECRITURE) $j("#bloc_accent").show();
			...
			$j("#toggle_media").click(function(){
				if(IS_MEDIA){
					...
					if(IS_VIDEO) $j("#toggle_waveform").hide();
					IS_MEDIA = false;
				}else{
					$j("#lecteurAudio_multimedia").show();
					if(IS_VIDEO) $j("#toggle_waveform").show();
					IS_MEDIA = true;	
				}
			});
...
function S_supprimerLigne(num){
	if(EN_TRAIN_DE_SUPPRIMER) return false;
...
function S_focusLigne(num,force){
	...
	if(IS_PLAYING == false || force == true){
		if( num >= 0 && num < S_nbLigne()){
			...
			if(ETAT == ETATS.ECRITURE) ligne.attr("contenteditable","true");
...
function S_retracerBlocSynchro(){
	for(i in BLOC_SYNCHRO.childNodes){	
			...
			var tailleBloc = 0;
			if(ETAT == ETATS.ECRITURE) blocCourant.resizable({ minHeight: HAUTEUR_MAX,maxHeight: HAUTEUR_MAX});
			i = parseInt(i);
			if(typeof BLOC_SYNCHRO.childNodes[i + 1] == "object"){//LE SUIVANT EXISTE
				...
				if(ETAT == ETATS.ECRITURE) blocCourant.resizable("enable");			
			}else{
				tailleBloc = BLOC_SYNCHRO.container - blocInfoCourant.position;
				if(ETAT == ETATS.ECRITURE) blocCourant.resizable("disable");
			}
...
function S_recupererHtml(){
	...
	$j('.textCol').each(function(i){
		...
		if(i < (nb - 1)) html += htmlspecialchars("<br>");
	});
	return html;
}
...
function S_recupererTexte(){
	...
	$j('.textCol').each(function(i){
		...
		if(i < (nb - 1)) txt += htmlspecialchars("<br>");
	});
	return txt;
}
